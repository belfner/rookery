...
class Config:
    ...
    def __init__(self) -> None:
        ...
        self.install_dir = self._get_path_from_env("ROOST_INSTALL_DIR", Path("/opt/roost-programs"))
        self.bin_dir = self._get_path_from_env("ROOST_BIN_DIR", Path("~/.local/bin").expanduser())
        self.desktop_dir = self._get_path_from_env(
            "ROOST_DESKTOP_DIR", Path("~/.local/share/applications").expanduser()
        )
        self.man_dir = self._get_path_from_env("ROOST_MAN_DIR", Path("~/.local/share/man").expanduser())
        self.temp_dir = self._get_path_from_env("ROOST_TEMP_DIR", Path("/tmp/roost"))
    ...
    def get_path_source(self, path: Path) -> tuple[PathSource, str]:
        ...
        env_vars = {
            "ROOST_INSTALL_DIR": self.install_dir,
            "ROOST_BIN_DIR": self.bin_dir,
            "ROOST_DESKTOP_DIR": self.desktop_dir,
            "ROOST_MAN_DIR": self.man_dir,
            "ROOST_TEMP_DIR": self.temp_dir,
        }
...
try:
    ...
    print("Example: ROOST_INSTALL_DIR=/opt/roost-programs\n", file=sys.stderr)
    sys.exit(1)
